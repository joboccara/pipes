...
TEST_CASE("output::transform")
{
    ...
    SECTION("operator>>=")
    {
        std::copy(begin(input), end(input), pipes::output::transform([](int i) { return i*2; }) >>= std::back_inserter(results));
        REQUIRE(results == expected);
    }
}
...
TEST_CASE("output::transform with several outputs")
{
    ...
    auto const multiply = pipes::output::transform([](int i) { return i*2; },
                                                    [](int i) { return i*3; },
                                                    [](int i) { return i*4; });
    ...
    SECTION("operator >>=")
    {
        auto const identity = pipes::output::transform([](auto&& value){ return FWD(value); });
