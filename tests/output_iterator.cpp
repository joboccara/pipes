...
TEST_CASE("output_iterator makes operator++ a no-op and increments in operator=")
{
    ...
    auto const isEven = fluent::make_output_filter([](int n) { return n % 2 == 0; });
    std::copy(begin(numbers), end(numbers), isEven(fluent::output(begin(results))));
...
TEST_CASE("output_iterator is compatible with smart iterators that don't change iteration")
{
    ...
    auto const times2 = fluent::make_output_transformer([](int n) { return n * 2; });
    std::copy(begin(numbers), end(numbers), times2(fluent::output(begin(results))));
...
TEST_CASE("output_iterator's iterator category should be std::output_iterator_tag")
{
    ...
    static_assert(std::is_same<decltype(fluent::output(begin(output)))::iterator_category,
